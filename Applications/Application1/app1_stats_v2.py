#!/usr/bin/env python3
# Role: Fisher's exact test comparing SAS vs EUR carrier frequency at the chr8 locus (Application 1, revised).
import argparse
import csv
import json
import math
...
def main():
    ...
    carrier_ids_path = os.path.join(args.out_dir, "app1_carrier_sample_ids.csv")

    with open(out_path, "w", newline="") as f, open(carrier_ids_path, "w", newline="") as f_ids:
        writer = csv.writer(f)
        ...
        writer_ids.writerow(["metric", "group", "sample"])

        for name, carrier_fn in metrics:
            sas_carriers = [s for s in sas_samples if carrier_fn(s)]
            eur_carriers = [s for s in eur_samples if carrier_fn(s)]
            a, b = len(sas_carriers), len(sas_samples) - len(sas_carriers)
            c, d = len(eur_carriers), len(eur_samples) - len(eur_carriers)

            for s in sas_carriers:
                writer_ids.writerow([name, "SAS", s])
            for s in eur_carriers:
                writer_ids.writerow([name, "EUR", s])
    ...
    print(f"\nSaved to: {out_path}")
    print(f"Carrier ( diff ): {carrier_ids_path}")
